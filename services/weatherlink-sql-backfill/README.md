...
This service replays messages from Kafka topics and materializes them to PostgreSQL using the same processing pipeline as `weatherlink-sql`, but with:
...
- **Separate consumer group**: Uses `weatherlink-sql-backfill` to avoid interfering with real-time materializer
...
./weatherlink-sql-backfill --topics weather.iss,weather.barometer

# Backfill from specific offset range
./weatherlink-sql-backfill --start-offset 0 --end-offset 10000

# Custom performance settings
./weatherlink-sql-backfill --workers 16 --batch-size 1000
...
# Docker command
docker compose -f docker-compose.infrastructure.yml -f docker-compose.yml run --rm \
  weatherlink-sql-backfill --metadata
```
...
1. **Topic Discovery**: For each configured topic:
   - Create Kafka reader with consumer group `weatherlink-sql-backfill`
...
Uses the same schema as `weatherlink-sql`:
...
**Performance vs weatherlink-sql**:
...
```bash
# Check logs
docker compose logs weatherlink-sql-backfill
...
- Backfill uses separate consumer group: `weatherlink-sql-backfill`
- Materializer uses: `weatherlink-sql`
- No conflicts should occur

To reset backfill consumer group:
```bash
docker exec roach-kafka kafka-consumer-groups --bootstrap-server localhost:29092 \
  --group weatherlink-sql-backfill --reset-offsets --to-earliest --all-topics --execute
```

## Comparison with Real-time Materializer

| Feature | weatherlink-sql | weatherlink-sql-backfill |
|---------|-------------------------|---------------------------|
| **Mode** | Continuous daemon | One-shot execution |
| **Start Offset** | LastOffset (new messages only) | Configurable (earliest/latest/specific) |
| **Consumer Group** | weatherlink-sql | weatherlink-sql-backfill |
...
- **weatherlink-sql**: Real-time Kafka→DB streaming
