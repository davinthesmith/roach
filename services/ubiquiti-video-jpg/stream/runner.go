...
// Runner captures frames from an RTSPS stream via ffmpeg and writes them to a directory.
type Runner struct {
	cameraID   string
	cameraName string
	outputDir  string // Per-camera directory (e.g. baseDir/camera_name)
	logLevel   string
}

// NewRunner creates a new stream runner for a single camera.
func NewRunner(cameraID, cameraName, outputDir, logLevel string) *Runner {
	return &Runner{
		cameraID:   cameraID,
...
// and writes each frame to a file. Blocks until ffmpeg exits or context is cancelled.
func (r *Runner) Run(ctx context.Context, rtspsURL string) error {
	...
	// Read frames from stdout and write to files
	...
	if ctx.Err() != nil {
		log.Printf("[%s] Stopped (context cancelled, %d frames written)", r.cameraName, frameCount)
		return ctx.Err()
	}
	...
	for {
		...
		if n > 0 {
			...
			for {
				...
				ts := time.Now().Unix()
				if writeErr := r.writeFrame(frame, ts); writeErr != nil {
					log.Printf("[%s] Write error: %v", r.cameraName, writeErr)
				} else {
					frameCount++
					if r.logLevel == "debug" {
						log.Printf("[%s] Frame %d written (%d bytes, ts=%d)",
							r.cameraName, frameCount, len(frame), ts)
					} else if frameCount%60 == 0 {
						log.Printf("[%s] %d frames written", r.cameraName, frameCount)
					}
				}
			}
...
// writeFrame writes a single JPEG frame to {outputDir}/{timestamp}.jpg.
func (r *Runner) writeFrame(frame []byte, timestamp int64) error {
	name := fmt.Sprintf("%d.jpg", timestamp)
