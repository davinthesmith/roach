...
This service replays messages from Kafka topics and materializes them to PostgreSQL using the same processing pipeline as `weatherlink-materialize-to-sql`, but with:
...
- **Separate consumer group**: Uses `weatherlink-backfill-to-sql` to avoid interfering with real-time materializer
...
./weatherlink-backfill-to-sql --topics weather.iss,weather.barometer

# Backfill from specific offset range
./weatherlink-backfill-to-sql --start-offset 0 --end-offset 10000

# Custom performance settings
./weatherlink-backfill-to-sql --workers 16 --batch-size 1000

# Backfill from latest (only new messages)
./weatherlink-backfill-to-sql --start-offset -1
```
...
docker compose -f docker-compose.infrastructure.yml -f docker-compose.yml run --rm weatherlink-backfill-to-sql --topics weather.iss
...
./scripts/weatherlink-backfill-to-sql.sh --metadata --topics ""

# Backfill metadata AND data topics (recommended for fresh database)
./scripts/weatherlink-backfill-to-sql.sh --metadata

# Docker command
docker compose -f docker-compose.infrastructure.yml -f docker-compose.yml run --rm \
  weatherlink-backfill-to-sql --metadata
```
...
1. **Topic Discovery**: For each configured topic:
   - Create Kafka reader with consumer group `weatherlink-backfill-to-sql`
...
Uses the same schema as `weatherlink-materialize-to-sql`:
...
docker compose logs -f weatherlink-backfill-to-sql
...
**Performance vs weatherlink-materialize-to-sql**:
...
docker compose stop weatherlink-materialize-to-sql
...
docker compose start weatherlink-materialize-to-sql
```
...
./scripts/weatherlink-backfill-to-sql.sh --topics weather.iss

# Backfill indoor and barometer data
./scripts/weatherlink-backfill-to-sql.sh --topics weather.indoor,weather.barometer
```
...
./scripts/weatherlink-backfill-to-sql.sh --topics weather.iss --start-offset 10000
```
...
./scripts/weatherlink-backfill-to-sql.sh --workers 16 --batch-size 1000
```
...
```bash
# Check logs
docker compose logs weatherlink-backfill-to-sql
...
- Backfill uses separate consumer group: `weatherlink-backfill-to-sql`
- Materializer uses: `weatherlink-materialize-to-sql`
- No conflicts should occur

To reset backfill consumer group:
```bash
docker exec roach-kafka kafka-consumer-groups --bootstrap-server localhost:29092 \
  --group weatherlink-backfill-to-sql --reset-offsets --to-earliest --all-topics --execute
```

## Comparison with Real-time Materializer

| Feature | weatherlink-materialize-to-sql | weatherlink-backfill-to-sql |
|---------|-------------------------|---------------------------|
| **Mode** | Continuous daemon | One-shot execution |
| **Start Offset** | LastOffset (new messages only) | Configurable (earliest/latest/specific) |
| **Consumer Group** | weatherlink-materialize-to-sql | weatherlink-backfill-to-sql |
...
- **weatherlink-materialize-to-sql**: Real-time Kafka→DB streaming
- **weatherlink-api-backfill**: API→Kafka historical backfill
- **weatherlink-ingest-to-kafka**: Real-time API→Kafka streaming
