...
```
WeatherLink API → weatherlink-ingest-to-kafka → Kafka Topics
                       ↓
                 Change Detection
                 Hash Comparison
```
...
```bash
# From project root
docker compose up weatherlink-ingest-to-kafka
```
...
```
services/weatherlink-ingest-to-kafka/
├── main.go              # Entry point
...
# Build Docker image
docker build -t roach-weatherlink-ingest-to-kafka .
```
...
```bash
# Check logs
docker compose logs weatherlink-ingest-to-kafka
...
4. Look for errors in logs: `docker compose logs -f weatherlink-ingest-to-kafka`
...
docker compose exec weatherlink-ingest-to-kafka nc -zv kafka 29092

# Test PostgreSQL connection
docker compose exec weatherlink-ingest-to-kafka nc -zv postgres 5432
...
docker compose logs -f weatherlink-ingest-to-kafka

# Check if service is running
docker compose ps weatherlink-ingest-to-kafka

# Restart service
docker compose restart weatherlink-ingest-to-kafka
```
