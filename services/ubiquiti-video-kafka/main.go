...
import (
	...
	"ubiquiti-video-kafka/api"
	"ubiquiti-video-kafka/config"
	"ubiquiti-video-kafka/kafka"
	"ubiquiti-video-kafka/service"
)

func main() {
	cfg := config.Load()
	log.Printf("Starting ubiquiti-video-kafka service (log_level=%s)", cfg.LogLevel)
