...
**Note**: This service backfills from WeatherLink API to Kafka. For Kafka→DB backfill, see `weatherlink-backfill-to-sql`.
...
./weatherlink-backfill-to-kafka --start 1768780863 --end 1768865863

# Backfill using datetime strings
./weatherlink-backfill-to-kafka --start "2026-01-11 18:20:47" --end "2026-01-12 18:20:47"

# Backfill from start to now
./weatherlink-backfill-to-kafka --start 1768780863

# Backfill from datetime to now
./weatherlink-backfill-to-kafka --start "2026-01-11 18:20:47"

# Custom rate limiting and parallelism
./weatherlink-backfill-to-kafka --start 1768780863 --requests-per-second 5 --workers 8
```
...
./scripts/weatherlink-backfill-to-kafka.sh --start "2026-01-11 18:20:47" --end "2026-01-12 18:20:47"

# Or with full command
docker compose -f docker-compose.infrastructure.yml -f docker-compose.yml run --rm weatherlink-backfill-to-kafka --start $(date -v-24H +%s)

# With custom args
./scripts/weatherlink-backfill-to-kafka.sh --start $(date -v-24H +%s) --end $(date +%s) --requests-per-second 5 --workers 8

# Using datetime format with date command
./scripts/weatherlink-backfill-to-kafka.sh --start "$(date -v-24H '+%Y-%m-%d %H:%M:%S')"
```
...
./scripts/weatherlink-backfill-to-kafka.sh --start $(date -v-7d +%s)
...
./scripts/weatherlink-backfill-to-kafka.sh --start "2026-01-11" --end "2026-01-12"

# Backfill with slower rate (if hitting limits)
./scripts/weatherlink-backfill-to-kafka.sh --start "2026-01-11 18:20:47" --requests-per-second 5

# Backfill with more workers for faster processing
./scripts/weatherlink-backfill-to-kafka.sh --start "2026-01-11" --workers 8
```
