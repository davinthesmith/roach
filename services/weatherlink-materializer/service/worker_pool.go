...
func (wp *WorkerPool) processMessage(msg kafka.Message) error {
	...
	if device == nil {
		return wp.orphanRepo.Save(context.Background(), msg, lsid, timestamp, "", "missing_device")
	}
	...
	for fieldName, fieldValue := range data {
		...
		if tag == nil {
			...
			if err != nil {
				...
				wp.orphanRepo.Save(context.Background(), msg, lsid, timestamp, fieldName, "failed_to_create_tag")
				continue
			}
		...
		// Add record to batch writer instead of inserting directly
